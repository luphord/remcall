...
class Type:
    ...
    @property
    def sort_key(self):
        assert self.type_order >= 0, ('Type {} has type_order {} < 0 and ' +
                                      'cannot be sorted') \
                                     .format(self.name, self.type_order)
...
def assert_type_or_ref(typ: TypeOrRef):
    error_msg = '{} which not an instance of Type or TypeRef'.format(typ)
    assert isinstance(typ, Type) or isinstance(typ, TypeRef), error_msg
...
class Array(Type):
    ...
    def __eq__(self, other: Type) -> bool:
        '''
        Equality of array types is equivalent
        to the equality of their underlying types
        >>> int64 == int64
        True
...
for typename in ('boolean', 'int8', 'uint8', 'int16', 'uint16',
                 'int32', 'uint32', 'int64', 'uint64',
                 'float32', 'float64', 'string', 'date', 'time', 'datetime'):
...
class Enum(Type):
    ...
    def pretty_print(self) -> str:
        ...
        return 'enum {} {{\n{}\n}}'.format(self.name, enum_values)

    def __repr__(self) -> str:
        return '{}(name="{}", values={})'.format(self.__class__.__name__,
...
class Record(Type):
    ...
    def __init__(self,
                 name: str,
                 fields: Iterable[Tuple[TypeOrRef, str]]) -> None:
        super().__init__(name)
    ...
    def resolve_type_references(self,
                                type_ref_lookup: Mapping[TypeRef, Type]) \
            -> None:
    ...
    def pretty_print(self) -> str:
        record_fields = '\n'.join('  {} {};'.format(field_type, field_name)
                                  for field_type, field_name in self.fields)
        return 'record {} {{\n{}\n}}'.format(self.name, record_fields)

    def __repr__(self) -> str:
        return '{}(name="{}", fields={})'.format(self.__class__.__name__,
...
class Method:
    def __init__(self,
                 name: str,
                 arguments: Iterable[Tuple[TypeOrRef, str]],
    ...
    def __str__(self) -> str:
        args = ', '.join('{!s} {}'.format(typ, name)
                         for typ, name in self.arguments)
        return '{!s} {}({});'.format(self.return_type, self.name, args)
        ...
        for method in self.methods:
            for arg_idx, arg in enumerate(list(method.arguments)):
                ...
                if isinstance(arg_type, TypeRef):
                    method.arguments[arg_idx] = (type_ref_lookup[arg_type],
                                                 arg_name)
            if isinstance(method.return_type, TypeRef):
                method.return_type = type_ref_lookup[method.return_type]

    def pretty_print(self) -> str:
        methods = '\n'.join('  {!s}'.format(method)
                            for method in self.methods_sorted)
        return 'interface {} {{\n{}\n}}'.format(self.name, methods)

    def __repr__(self) -> str:
        return '{}(name="{}", methods={!r})'.format(self.__class__.__name__,
                                                    self.name, self.methods)


class Schema:
    def __init__(self, label, types,
                 bytes_method_ref=2, bytes_object_ref=4,
                 sha256_digest=None):
        self.label = label
        ...
        if sha256_digest:
            assert len(sha256_digest) == 32, \
                   'SHA256 digest must be 32 bytes long'
        self.sha256_digest = sha256_digest
        assert hasattr(self.type_schemas, 'Main'), \
            ('Every schema requires an interface called "Main", ' +
             'got only {}') \
            .format(', '.join('"{}"'.format(ifc.name)
                              for ifc in self.interfaces_sorted))
        self.main_type = self.type_schemas.Main
        for ifc in self.interfaces_sorted:
            assert len(ifc.methods) > 0, \
                   ('Every interface requires at least one method, ' +
                    '"{}" has none').format(ifc.name)
    ...
    @property
    def type_table(self):
        assert primitive_types[0].name == 'void', \
               'First type with index 0 has to be void, got {}' \
               .format(primitive_types[0])
        table = {typ: idx for idx, typ in
                 enumerate(primitive_types + self.declared_types)}
    ...
    def method_lookup(self):
        methods = sum((iface.methods_sorted
                       for iface in self.interfaces_sorted), [])
        return {idx: method for idx, method in enumerate(methods)}
    ...
    def __repr__(self):
        return '{}(label="{}", types={!r})'.format(self.__class__.__name__,
                                                   self.label, self.types)


__all__ = ['Type', 'Array', 'Primitive', 'Enum', 'Record', 'Method',
           'Interface', 'Schema', 'primitive_types'] + \
          [tp.name for tp in primitive_types]
